...
countcgs <-  function(x)
{
  tmp <- intersect(unlist(strsplit(x[5], ";")), intersect(unlist(strsplit(x[10], ";")), intersect(unlist(strsplit(x[15], ";")), unlist(strsplit(x[20], ";")))))
  return(tmp)
}
...
{
  tmp <- c(unlist(strsplit(x[5], ";")), unlist(strsplit(x[10], ";")), unlist(strsplit(x[15], ";")), unlist(strsplit(x[20], ";")))
...
covariatesummary <- read.table("I.Data_Processing/Tables/covariateTumorPuritySummary.csv", sep = ",", row.names = 1, header = T)
...
{
  AdjustedList[[i]] <- read.table(paste(file.loc,AdjustedFiles[[i]], sep = ""), header = T, sep = ",", stringsAsFactors = F)
...
{
  UnadjustedList[[i]] <- read.table(paste(file.loc, UnadjustedFiles[[i]], sep = ""), header = T, sep = ",", stringsAsFactors = F)
...
for(i in 1:length(subtypes))
{
  ...
  MasterList[[i]] <- list("High" = High, "Low" = Low, "High-Only" = HighOnly, "Low-Only" = LowOnly, "Overlap" = list("OverlapHigh" = OverlapHigh, "OverlapLow" = OverlapLow))
...
for(i in 1:length(summaryModelList))
{
  ...
  #Write tiff figure to disk
  tiff(filename = paste("III.DMGR_analysis/Figures/", names(summaryModelList)[i], "_FullAnnotation.tiff", sep = ""), compression = "lzw", width = 700, height = 700)
  vennDiagram(venn, main = paste("\nOverlapping Gene Regions across Stage: \nFull Annotation -", subtypes[i], sep = ""))
...
#   #Get text for plot
#   texting <- c()
#   for(j in 1:(nrow(subs)-1))
#   {
#     number <- paste("n =", subs[j,1])
#     string <- paste(" Tumor Mean =", subs[j,2], "\nTumor SD =", subs[j,3])
#     texting <- c(texting, number, string)
#   }
  
  #initialize a venn matrix to get ready for overlaps (-1 subtype because we are not considering Normal-Like for this analysis)
  ...
  vennReady <- vennCounts(Venn)
  
  #Get venn diagram plot
  png(filename = paste("III.DMGR_analysis/Figures/", stage[k], "FullAnnotation_noNormals.png", sep = ""), width = 300, height = 550)
  vennDiagram(vennReady)
  #text(x = c(40,40,145,140,265,265,350,370), y = c(20,350,20,375,20,375,20,350),  labels = texting)
  dev.off()
}
...
colnames(commonSummary) <- c("Basal_sign", "Basal_q", "Basal_B", "Basal_D", "Basal_cpgs","Her2_sign", "Her2_q", "Her2_B", "Her2_D", "Her2_cpgs", "LumA_sign", "LumA_q", "LumA_B", "LumA_D","LumA_cpgs", "LumB_sign", "LumB_q", "LumB_B", "LumB_D", "LumB_cpgs", "Denominator")
...
commonSummary <- cbind(commonSummary, int[match(rownames(commonSummary), names(int))], uni[match(rownames(commonSummary), names(uni))])
