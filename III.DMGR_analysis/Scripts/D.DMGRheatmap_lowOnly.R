...
source("III.DMGR_analysis/Scripts/Functions/make_heatmaps_lowOnly.R")
...
covariates <- covariates[covariates$PAM50.RNAseq != "Normal", ]
...
covariates <- covariates[covariates$pathologic_stage == "low" | covariates$pathologic_stage == "normal",]
...
# Save to disk
png(filename = "III.DMGR_analysis/Figures/heatmaps/Heatmap_LowOverlappingCpGs_0.01_lowOnly.png", 
    width = 1800, height = 1400)
CGHeatmap_low("Common Low Stage CpGs\n (q value cutoff = 0.01)", lowcgs, regions, annotationGR, 
          #annotationMap, Betas, covariates, stages = c("low", "normal"), Full = T, Validation = F)
          annotationMap, Betas, covariates, stages = c("low"), Full = T, Validation = F)
dev.off()
