...
covariatesummary <- read.table("I.Data_Processing/Tables/covariateTumorPuritySummary.csv", sep = ",", 
...
for (i in 1:length(AdjustedFiles)) {
  AdjustedList[[i]] <- read.table(paste(file.loc,AdjustedFiles[[i]], sep = ""), 
...
for (i in 1:length(UnadjustedFiles)) {
  UnadjustedList[[i]] <- read.table(paste(file.loc, UnadjustedFiles[[i]], sep = ""), header = T, 
...
for (i in 1:length(summaryModelList)) {
  ...
  tiff(filename = paste("III.DMGR_analysis/Figures/", names(summaryModelList)[i], "_FullAnnotation.tiff", sep = ""), 
       compression = "lzw", width = 700, height = 700)
...
for (k in 1:length(stage)) {
  ...
  # Get text for plot
  total_cpg <- sum(vennReady[2:nrow(vennReady), ncol(vennReady)])
  texting <- paste("n = ", total_cpg)
  
  # Get venn diagram plot
  png(filename = paste("III.DMGR_analysis/Figures/", stage[k], "FullAnnotation_noNormals.png", sep = ""), 
       width = 430, height = 480)
  vennDiagram(vennReady, main = paste("\nOverlapping Gene Regions: \nFull Annotation File - ", 
                                      stage[k]," Stage", sep = ""))
  text(x = 40, y = 20,  labels = texting)
  dev.off()
}
...
#Write to file
write.table(commonSummary, file = "III.DMGR_analysis/Tables/commonLowStageOverlaps_FullAnnotation_extended.csv",
...
write.table(commonSummaryHigh, file = "III.DMGR_analysis/Tables/commonLowStageOverlaps_HighStage.csv", 
            sep = ",", row.names = T, col.names = NA)
