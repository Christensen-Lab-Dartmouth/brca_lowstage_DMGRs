...
beta2 <- read.table(beta.file, row.names = 1, header = T, sep = "\t", stringsAsFactors = F, nrows = 400000, 
                    comment.char = "")
...
for (j in 1:length(anno.sub)) {
  ...
  write.table(methylatedFrame, file = paste("III.DMGR_analysis/Results/DMR_", names(anno.sub)[j], qvalcut, ".csv", sep = ""), row.names = F, sep = ",")
}